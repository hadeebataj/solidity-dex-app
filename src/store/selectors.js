...
export const myOpenOrdersSelector = createSelector(
  ...
  (account, tokens, orders) => {
    ...
    // Filter orders created by token addresses
...
const decorateOrder = (order, tokens) => {
  ...
  // Note: mRUPC is considered token0, mWETH is considered token1
  // Example : Giving mWETH in exchange for mRUPC
  if (order.takenGive === tokens[1].address) {
    token0Amount = order.amountGive; // The amount of mRUPC we are giving
    token1Amount = order.amountGet; // The amount of mWETH we want
  } else {
    token0Amount = order.amountGet; // The amount of mRUPC we want
...
export const filledOrdersSelector = createSelector(
  ...
  (orders, tokens) => {
    ...
    // Step 1: sort orders by time ascendning
    ...
    // Sort orders by date descending for display (UI)
    orders = orders.sort((a, b) => b.timestamp - a.timestamp);
